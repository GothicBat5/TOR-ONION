...
sock_alert(tor_socket_t fd)
{
  ...
  if (r < 0 && !ERRNO_IS_EAGAIN(-r)) return (int)r; return 0;
}
static int
...
alert_sockets_create(alert_sockets_t *socks_out, uint32_t flags)
{
  ...
  if (!(flags & ASOCKS_NOEVENTFD2))  socks[0] = eventfd(0, EFD_CLOEXEC|EFD_NONBLOCK);
#endif
  if (socks[0] < 0 && !(flags & ASOCKS_NOEVENTFD)) 
  {
    socks[0] = eventfd(0,0);
    if (socks[0] >= 0) {
      if (fcntl(socks[0], F_SETFD, FD_CLOEXEC) < 0 || set_socket_nonblocking(socks[0]) < 0) 
      {
  ...
  if (!(flags & ASOCKS_NOSOCKETPAIR) &&
    ...
    if (set_socket_nonblocking(socks[0]) < 0 || set_socket_nonblocking(socks[1])) {
      tor_assert_nonfatal_unreached();
