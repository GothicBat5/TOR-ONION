...
void GetTimeDeltaFromDictString(const base::DictValue& args, std::string_view key, base::TimeDelta* out) 
{
...
HostResolver::HttpsSvcbOptions HostResolver::HttpsSvcbOptions::FromDict(
  ...
  options.insecure_extra_time_percent = dict.FindInt(kUseDnsHttpsSvcbInsecureExtraTimePercent)
  ...
  options.secure_extra_time_percent = dict.FindInt(kUseDnsHttpsSvcbSecureExtraTimePercent)
...
std::unique_ptr<HostResolver> HostResolver::Factory::CreateResolver(
    ...
    bool enable_caching,
    bool enable_stale) {
  return HostResolver::CreateResolver(manager, host_mapping_rules, enable_caching, enable_stale);
}
...
HostResolver::ResolveHostParameters::ResolveHostParameters(const ResolveHostParameters&) = default;

HostResolver::ResolveHostParameters&
HostResolver::ResolveHostParameters::operator=(const ResolveHostParameters&) = default;
...
std::unique_ptr<HostResolver> HostResolver::CreateResolver(
  ...
  context_resolver = std::make_unique<ContextHostResolver>(manager, std::move(resolve_context));

  std::unique_ptr<HostResolver> resolver;

  if (enable_stale) {
    resolver = std::make_unique<StaleHostResolver>(std::move(context_resolver), StaleHostResolver::StaleOptions());
  } else {
...
std::unique_ptr<HostResolver> HostResolver::CreateStandaloneResolver(
    ...
    bool enable_stale) {
  std::unique_ptr<ContextHostResolver> 
  context_resolver = CreateStandaloneContextResolver(net_log, std::move(options), enable_caching);
...
HostResolver::CreateStandaloneNetworkBoundResolver(
  ...
  bool is_builtin_resolver_supported = manager_options.insecure_dns_client_enabled &&
      base::android::android_info::sdk_int() >= base::android::android_info::SDK_VERSION_P;
