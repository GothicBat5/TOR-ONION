...
SkBitmap ReadPixels(SkCanvas* canvas) {
  ...
  if (!canvas->readPixels(bitmap, 0, 0)) bitmap.reset();
  return bitmap;
}
...
std::unique_ptr<SkCanvas> CreatePlatformCanvasWithPixels(int width, int height, bool is_opaque,
    uint8_t* data, size_t bytes_per_row,
    OnFailureType failureType) 
    {
      SkBitmap bitmap;
      bitmap.setInfo(SkImageInfo::MakeN32(width, height, is_opaque ? kOpaque_SkAlphaType : kPremul_SkAlphaType), bytes_per_row);
