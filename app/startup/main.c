...
do_hup(void)
{
  ...
  log_notice(LD_GENERAL,"Received reload signal (hup). Reloading config and " "resetting internal state.");
  if (accounting_is_enabled(options)) accounting_record_bandwidth_usage(time(NULL), get_or_state());
  ...
  if (options->ReloadTorrcOnSIGHUP) {
    ...
    if (init_rv < 0) {
      log_err(LD_CONFIG,"Reading config failed--see warnings above. " "For usage, try -h.");
      return -1;
    } 
    ...
    if (options->TruncateLogFile) truncate_logs();
  } 
  else {
    char *msg = NULL;
    log_notice(LD_GENERAL, "Not reloading config file: the controller told " "us not to.");
  ...
  if (!net_is_disabled()) update_networkstatus_downloads(time(NULL));

  if (server_mode(options)) {
    time_t now = approx_time();
    int new_signing_key = load_ed_keys(options, now);
    if (new_signing_key < 0 || generate_ed_link_cert(options, now, new_signing_key > 0)) 
    {
      log_warn(LD_OR, "Problem reloading Ed25519 keys; still using old keys.");
    }
    if (load_family_id_keys(options, networkstatus_get_latest_consensus())) {
      log_warn(LD_OR, "Problem reloading family ID keys; "  "still using old keys.");
    }
...
dumpmemusage(int severity)
{
  ...
  tor_log(severity, LD_GENERAL, "In rephist: %"PRIu64" used by %d Tors.", (rephist_total_alloc), rephist_total_num);
...
dumpstats(int severity)
{
  ...
  {
    int i = conn_sl_idx;
    tor_log(severity, LD_GENERAL, "Conn %d (socket %d) is a %s, created %d secs ago",i, (int)conn->s, connection_describe(conn), (int)(now - conn->timestamp_created));
    if (!connection_is_listener(conn)) {
      tor_log(severity,LD_GENERAL,
          "Conn %d: %d bytes waiting on inbuf (len %d, last read %d secs ago)", i,(int)connection_get_inbuf_len(conn), (int)buf_allocation(conn->inbuf), (int)(now - conn->timestamp_last_read_allowed));
      tor_log(severity,LD_GENERAL, "Conn %d: %d bytes waiting on outbuf " "(len %d, last written %d secs ago)",i,(int)connection_get_outbuf_len(conn),(int)buf_allocation(conn->outbuf), (int)(now - conn->timestamp_last_write_allowed));
      if (conn->type == CONN_TYPE_OR) 
      {
        or_connection_t *or_conn = TO_OR_CONN(conn);
        if (or_conn->tls)
        {
          if (tor_tls_get_buffer_sizes(or_conn->tls, &rbuf_cap, &rbuf_len, &wbuf_cap, &wbuf_len) == 0) 
          {
            tor_log(severity, LD_GENERAL, "Conn %d: %d/%d bytes used on OpenSSL read buffer; " "%d/%d bytes used on write buffer.", i, (int)rbuf_len, (int)rbuf_cap, (int)wbuf_len, (int)wbuf_cap);
          }
        }
      }
    }
  ...
  if (stats_n_data_cells_packaged) tor_log(severity,LD_NET,"Average packaged cell fullness: %2.3f%%", 100*(((double)stats_n_data_bytes_packaged) / ((double)stats_n_data_cells_packaged*RELAY_PAYLOAD_SIZE_MAX)) );
  if (stats_n_data_cells_received)
    tor_log(severity,LD_NET,"Average delivered cell fullness: %2.3f%%", 100*(((double)stats_n_data_bytes_received) /((double)stats_n_data_cells_received*RELAY_PAYLOAD_SIZE_MAX)) );
  ...
  if (now - time_of_process_start >= 0) elapsed = now - time_of_process_start;
  else elapsed = 0;
  if (elapsed) 
  {
    tor_log(severity, LD_NET, "Average bandwidth: %"PRIu64"/%d = %d bytes/sec reading", (get_bytes_read()),(int)elapsed, (int) (get_bytes_read()/elapsed));
    tor_log(severity, LD_NET, "Average bandwidth: %"PRIu64"/%d = %d bytes/sec writing", (get_bytes_written()), (int)elapsed, (int) (get_bytes_written()/elapsed));
  }
  ...
  for (i = 0; signal_handlers[i].signal_value >= 0; ++i)
  {
    if (enabled && signal_handlers[i].try_to_register) 
    {
      signal_handlers[i].signal_event =
        tor_evsignal_new(tor_libevent_get_base(), signal_handlers[i].signal_value, signal_callback,  &signal_handlers[i].signal_value);
      if (event_add(signal_handlers[i].signal_event, NULL))log_warn(LD_BUG, "Error from libevent when adding "  "event for signal %d",  signal_handlers[i].signal_value);
    } 
    else {
      signal_handlers[i].signal_event = tor_event_new(tor_libevent_get_base(), -1, EV_SIGNAL, signal_callback, &signal_handlers[i].signal_value);
    }
  }
...
tor_init(int argc, char *argv[])
{
...

  {
    ...
    log_notice(LD_GENERAL, "Tor %s running on %s with Libevent %s, "  "%s %s, Zlib %s, Liblzma %s, Libzstd %s and %s %s as libc.",
               version,
               get_uname(),
    ...
    if (strstr(version, "alpha") || strstr(version, "beta"))  log_notice(LD_GENERAL, "This version is not a stable Tor release. " "Expect more bugs than usual.");
    if (strlen(risky_option_list) && running_tor) {
      log_warn(LD_GENERAL, "This build of Tor has been compiled with one " "or more options that might make it less reliable or secure! " "They are:%s", risky_option_list);
    }
...
#ifndef _WIN32
  if (geteuid()==0) log_warn(LD_GENERAL,"You are running Tor as root. You don't need to, " "and you probably shouldn't.");
#endif
  routerparse_init();
  return 0;
}
  ...
  if (lockfile)
    ...
    if (!lf) {
      if (err_if_locked && already_locked) {
        int r;
        log_warn(LD_GENERAL, "It looks like another Tor process is running " "with the same data directory.  Waiting 5 seconds to see " "if it goes away.");
#ifndef _WIN32
        sleep(5);
#else
        Sleep(5000);
...
do_list_fingerprint(void)
{
  ...
  if (!show_rsa && !show_ed25519) {
    log_err(LD_GENERAL, "If you give a key type, you must specify 'rsa' or 'ed25519'. Exiting.");
    return -1;
  }

  if (!server_mode(options)) {
    log_err(LD_GENERAL, "Clients don't have long-term identity keys. Exiting.");
    return -1;
  }
  tor_assert(nickname);
  if (init_keys() < 0) {
...
do_hash_password(void)
{
  ...
  secret_to_key_rfc2440(key+S2K_RFC2440_SPECIFIER_LEN, DIGEST_LEN, get_options()->command_arg, strlen(get_options()->command_arg), key);
...
do_keygen_family(const char *fname_base)
{
  ...
  if (BUG(!fname_base)) goto done
  tor_asprintf(&fname_key, "%s.secret_family_key", fname_base);
  tor_asprintf(&fname_id, "%s.public_family_id", fname_base);
  if (create_family_id_key(fname_key, &pk) < 0) goto done;
  tor_asprintf(&id_contents, "%s\n", ed25519_fmt(&pk));
  if (write_str_to_file(fname_id, id_contents, 0) < 0) goto done;
  printf("# Generated %s\n", fname_key);
  printf("FamilyId %s\n", ed25519_fmt(&pk));  r = 0;
 done:
  tor_free(fname_key);
...
do_keygen_onion_key(const char *fname)
{
  ...
  if (BUG(! fname)) goto done;
  if (curve25519_keypair_generate(&keys, /* extra strong */ 1) < 0) goto done;
  if (curve25519_keypair_write_to_file(&keys, fname, "onion") < 0) goto done;
...
#define OPEN_DATADIR_SUFFIX(name, suffix) do { \ OPEN_DATADIR(name); \ OPEN_DATADIR(name suffix); \ } while (0)
#define OPEN_DATADIR2_SUFFIX(name, name2, suffix) do { \  OPEN_DATADIR2(name, name2); \ OPEN_DATADIR2(name, name2 suffix); \ } while (0)
...
#define OPEN_CACHEDIR_SUFFIX(name, suffix) do { \ OPEN_CACHEDIR(name); \ OPEN_CACHEDIR(name suffix); \ } while (0)
#define OPEN_KEYDIR(name) \ sandbox_cfg_allow_open_filename(&cfg, get_keydir_fname(name))
#define OPEN_KEYDIR_SUFFIX(name, suffix) do { \ OPEN_KEYDIR(name); \ OPEN_KEYDIR(name suffix); \ } while (0)
  ...
  {
    smartlist_t *family_id_files =  list_family_key_files(options, options->FamilyKeyDirectory);
    SMARTLIST_FOREACH(family_id_files, const char *, fn, OPEN(fn));
    SMARTLIST_FOREACH(family_id_files, char *, cp, tor_free(cp));
    smartlist_free(family_id_files);
  }
#endif

  if (options->BridgeAuthoritativeDir) OPEN_DATADIR_SUFFIX("networkstatus-bridges", ".tmp");
  if (authdir_mode(options)) 
  {
    ...
    {
      log_notice(LD_GENERAL, "Adding V3BandwidthsFile %s to sandboxing set.", options->V3BandwidthsFile);
      OPEN(options->V3BandwidthsFile);
    }
  }
  if (options->ServerDNSResolvConfFile)
    sandbox_cfg_allow_open_filename(&cfg, tor_strdup(options->ServerDNSResolvConfFile));
  else sandbox_cfg_allow_open_filename(&cfg, tor_strdup("/etc/resolv.conf"));
  const char *torrc_defaults_fname = get_torrc_fname(1);
  if (torrc_defaults_fname)  sandbox_cfg_allow_open_filename(&cfg, tor_strdup(torrc_defaults_fname));
...
#define RENAME_SUFFIX(name, suffix) \ sandbox_cfg_allow_rename(&cfg, \get_datadir_fname(name suffix),      \
      get_datadir_fname(name))
#define RENAME_SUFFIX2(prefix, name, suffix) \
  sandbox_cfg_allow_rename(&cfg,  \ get_datadir_fname2(prefix, name suffix), \ get_datadir_fname2(prefix, name))
#define RENAME_CACHEDIR_SUFFIX(name, suffix) \ sandbox_cfg_allow_rename(&cfg, \ get_cachedir_fname(name suffix), \ get_cachedir_fname(name))
#define RENAME_KEYDIR_SUFFIX(name, suffix) \ sandbox_cfg_allow_rename(&cfg, \ get_keydir_fname(name suffix), \  get_keydir_fname(name))
  ...
  if (options->BridgeAuthoritativeDir)  RENAME_SUFFIX("networkstatus-bridges", ".tmp");
  if (authdir_mode(options)) {
...
#define STAT_KEY_DIRECTORY() \
...

  {
    ...
    SMARTLIST_FOREACH(files, char *, file_name, {
      ...
      sandbox_cfg_allow_rename(&cfg, tor_strdup(tmp_name), tor_strdup(file_name));
      sandbox_cfg_allow_open_filename(&cfg, file_name);
  ...
  {
    char *fname;
    if ((fname = get_controller_cookie_file_name())) { sandbox_cfg_allow_open_filename(&cfg, fname);
    }
    if ((fname = get_ext_or_auth_cookie_file_name())) { sandbox_cfg_allow_open_filename(&cfg, fname);
    }
  }

  SMARTLIST_FOREACH_BEGIN(get_configured_ports(), port_cfg_t *, port) {
    if (!port->is_unix_addr) continue;
  ...
  if (options->DirPortFrontPage) {
    sandbox_cfg_allow_open_filename(&cfg, tor_strdup(options->DirPortFrontPage));
  }
  if (server_mode(get_options())) {
    ...
    sandbox_cfg_allow_rename(&cfg, get_keydir_fname("secret_onion_key"), get_keydir_fname("secret_onion_key.old"));
    sandbox_cfg_allow_rename(&cfg, get_keydir_fname("secret_onion_key_ntor"), get_keydir_fname("secret_onion_key_ntor.old"));
    STAT_KEY_DIRECTORY();
    OPEN_DATADIR("stats");
    STAT_DATADIR("stats");
...
run_tor_main_loop(void)
{
  ...
  if (trusted_dirs_reload_certs()) {
    log_warn(LD_DIR, "Couldn't load all cached v3 certificates. Starting anyway.");
  }
  if (router_reload_consensus_networkstatus())  return -1;
  if (router_reload_router_list())   return -1;
  const time_t now = time(NULL);
  directory_info_has_arrived(now, 1, 0);
  if (cpuworker_init() == -1) return -1;
  ...
  if (dns_init() < 0) {
    if (get_options()->ServerDNSAllowBrokenConfig)
      log_warn(LD_GENERAL, "Couldn't set up any working nameservers. " "Network not up yet?  Will try again soon.");
    else {
      log_err(LD_GENERAL,"Error initializing dns subsystem; exiting.  To " "retry instead, set the ServerDNSAllowBrokenResolvConf option.");
    }
  }
...
tor_run_main(const tor_main_configuration_t *tor_cfg)
{
  ...
  if (tor_cfg->argc_owned) memcpy(argv + tor_cfg->argc, tor_cfg->argv_owned, tor_cfg->argc_owned*sizeof(char*));
  int done = 0;
  result = nt_service_parse_options(argc, argv, &done);
  if (POSSIBLE(done)) goto done;
  pubsub_install();
  {
  ...
  if (get_options()->Sandbox && get_options()->command == CMD_RUN_TOR) {
    ...
    evutil_secure_rng_set_urandom_device_file((char*) sandbox_intern_string("/dev/urandom"));
#endif
  }
  ...
  default:
    log_warn(LD_BUG,"Illegal command number %d: internal error.", get_options()->command);
    result = -1;
  }
  tor_cleanup();
 done:
