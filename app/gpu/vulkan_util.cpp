...
int GetEMUIVersion() {
  ...
  if (base::android::android_info::manufacturer() != "HUAWEI")  return -1;

  int version = 0;
  if (UNSAFE_TODO(sscanf(base::android::android_info::version_incremental().c_str(),
      "%d.", &version)) != 1) {
    return -1;
  }
...
VulkanPhysicalDeviceProperties::VulkanPhysicalDeviceProperties(const VkPhysicalDeviceProperties& properties)
    : driver_version(properties.driverVersion),vendor_id(properties.vendorID), device_id(properties.deviceID),
...
{
  return SubmitSignalVkSemaphores(vk_queue, UNSAFE_TODO(base::span<VkSemaphore>(&vk_semaphore, 1u)),
      vk_fence);
}
...
{
  return SubmitWaitVkSemaphores(vk_queue, UNSAFE_TODO(base::span<VkSemaphore>(&vk_semaphore, 1u)),
      vk_fence);
}
...
VkResult CreateGraphicsPipelinesHook(VkDevice device, VkPipelineCache pipelineCache,
...
VkResult VulkanQueueSubmitHook(VkQueue queue, uint32_t submitCount,
...
bool CheckVulkanCompatibilities(const VulkanPhysicalDeviceProperties& device_properties,
  ...
  if (device_properties.vendor_id == kVendorARM) {
    ...
    if (!IsDeviceBlocked(gpu_info.gl_renderer, "*Mali-G?? M*"))  return true;
    if (device_name == "G57")  return false;
  ...
  if (device_properties.vendor_id == kVendorIntel) return false;
...
bool IsVkExternalSemaphoreHandleTypeSupported(VulkanDeviceQueue* device_queue, VkExternalSemaphoreHandleTypeFlagBits handle_type) 
{
  ...
  return (semaphore_properties.externalSemaphoreFeatures & VK_EXTERNAL_SEMAPHORE_FEATURE_EXPORTABLE_BIT) &&
