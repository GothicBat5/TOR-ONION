...
SkPictureBackedGlyphDrawable::MakeFromBuffer(SkReadBuffer& buffer) 
{
    ...
    if (!buffer.isValid() || pictureData->empty()) return nullptr;

    SkDeserialProcs procs;
    procs.fAllowSkSL = false;
    sk_sp<SkPicture> picture = SkPicture::MakeFromData(pictureData.get(), &procs);
  
    if (!buffer.validate(picture != nullptr)) return nullptr;
...
SkPictureBackedGlyphDrawable::SkPictureBackedGlyphDrawable(sk_sp<SkPicture> picture) : fPicture(std::move(picture)) {}
...
std::optional<SkGlyph> SkGlyph::MakeFromBuffer(SkReadBuffer& buffer) 
{
    ...
    if (!buffer.validate(SkMask::IsValidFormat(format))) return std::nullopt;
...
size_t SkGlyph::setMetricsAndImage(SkArenaAlloc* alloc, const SkGlyph& from) 
{
    ...
    if (fImage == nullptr) 
    {
        ...
        if (from.fImage != nullptr && this->setImage(alloc, from.image()))  return this->imageSize();
...
size_t SkGlyph::imageSize() const {
    ...
    if (fMaskFormat == SkMask::k3D_Format)  size *= 3;
...
{
    SkASSERT(this->setPathHasBeenCalled());
    if (fPathData->fHasPath)  return &fPathData->fPath;
    return nullptr;
}
...
void SkGlyph::flattenImage(SkWriteBuffer& buffer) const {
    ...
    if (!this->isEmpty() && SkGlyphDigest::FitsInAtlas(*this))  buffer.writeByteArray(this->image(), this->imageSize());
}

size_t SkGlyph::addImageFromBuffer(SkReadBuffer& buffer, SkArenaAlloc* alloc) {
    SkASSERT(buffer.isValid());

    if (this->isEmpty() || !SkGlyphDigest::FitsInAtlas(*this))  return 0;
...
size_t SkGlyph::addPathFromBuffer(SkReadBuffer& buffer, SkArenaAlloc* alloc) {
    ...
    if (hasPath) 
    {
        ...
        {
            if (this->setPath(alloc, &path.value(), pathIsHairline, pathIsModified)) memoryIncrease += path->approximateBytesUsed();
        }
    } 
    else {
...
size_t SkGlyph::addDrawableFromBuffer(SkReadBuffer& buffer, SkArenaAlloc* alloc) {
    ...
    if (this->setDrawable(alloc, std::move(drawable))) return this->drawable()->approximateBytesUsed();
    return 0;
}
...
static std::tuple<SkScalar, SkScalar> calculate_path_gap(SkScalar topOffset, SkScalar bottomOffset, const SkPath& path) 
{
    ...
    auto addLine = [&](SkSpan<const SkPoint> pts, SkScalar offset) 
  {
    ...
    auto addQuad = [&](SkSpan<const SkPoint> pts, SkScalar offset) {
        ...
        auto intersections = SkBezierQuad::IntersectWithHorizontalLine(pts, offset, intersectionStorage);
    ...
    auto addCubic = [&](SkSpan<const SkPoint> pts, SkScalar offset) 
  {
        ...
        auto intersections = SkBezierCubic::IntersectWithHorizontalLine(pts, offset, intersectionStorage);
    ...
    auto addPts = [&expandGap, topOffset, bottomOffset](SkSpan<const SkPoint> pts) 
    {
        ...
        {
            if (topOffset < p.fY && p.fY < bottomOffset)  expandGap(p.fX);
        }
    };
...
void SkGlyph::ensureIntercepts(const SkScalar* bounds, SkScalar scale, SkScalar xPos, SkScalar* array, int* count, SkArenaAlloc* alloc) 
{
  ...
  {
                if (fPathData == nullptr)   return nullptr;
                const SkGlyph::Intercept* intercept = fPathData->fIntercept;
                while (intercept != nullptr)
                {
                    if (bounds[0] == intercept->fBounds[0] && bounds[1] == intercept->fBounds[1])  return intercept;
    ...
    if (pathBounds.fBottom < bounds[0] || bounds[1] < pathBounds.fTop)  return;
